...
class Decoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1)  # 调整通道到 256
    ...
    def forward(self, rgb_list, depth_list, rgbd):
        ...
        """
        # rgb decoder stream
        rgb_3 = F.interpolate(rgb_list[3], scale_factor=2, mode='bilinear')
        rgb_3 = self.conv(rgb_3)
        rgb_block3 = self.r3(rgb_3, rgb_list[2])  # [B, 128, 44, 44]
        ...
        depth_3 = F.interpolate(depth_list[3], scale_factor=2, mode='bilinear')
        depth_3 = self.conv(depth_3)
        depth_block3 = self.d3(depth_3, depth_list[2])
        ...
        # rgbd decoder stream
        rgbd = self.conv(rgbd)
        rgbd = F.interpolate(rgbd, scale_factor=4, mode='bilinear', align_corners=True)
        rgbd_block3 = self.rd3(rgbd, rgb_block3, depth_block3)
...
rgb_list = [
    torch.randn(16, 64, 176, 176),   # 对应 decoder_rgb_list[0]
    torch.randn(16, 128, 88, 88),   # 对应 decoder_rgb_list[1]
    torch.randn(16, 256, 44, 44),   # 对应 decoder_rgb_list[2]
    torch.randn(16, 512, 22, 22),   # 对应 decoder_rgb_list[3]
    torch.randn(16, 512, 11, 11),   # 对应 decoder_rgb_list[4]
    torch.randn(16, 512, 11, 11),   # 对应 decoder_rgb_list[5]
]

depth_list = [
    torch.randn(16, 64, 176, 176),  # 对应 decoder_depth_list[0]
    torch.randn(16, 128, 88, 88),  # 对应 decoder_depth_list[1]
    torch.randn(16, 256, 44, 44),  # 对应 decoder_depth_list[2]
    torch.randn(16, 512, 22, 22),  # 对应 decoder_depth_list[3]
    torch.randn(16, 512, 11, 11),  # 对应 decoder_depth_list[4]
    torch.randn(16, 512, 11, 11),  # 对应 decoder_depth_list[5]
]
rgbd = torch.randn(16, 512, 11, 11)

decoder = Decoder()
rgb_map, depth_map, rgbd_map = decoder(rgb_list, depth_list, rgbd)


# print(f"rgb_map.shape: {rgb_map.shape}, depth_map.shape: {depth_map.shape}, rgbd_map.shape: {rgbd_map.shape}")
# # 定义融合模块
