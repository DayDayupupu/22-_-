...
class CIRNet_Dense121(nn.Module):
    ...
    def __init__(self, backbone='resnet50', norm_layer=nn.BatchNorm2d):
        # 
        super(CIRNet_Dense121, self).__init__()
        (
            self.rgb_block1,
            self.rgb_block2,
            self.rgb_block3,
            self.rgb_block4,
            self.rgb_block5,
        ) = Backbone_DenseNet121(pretrained=True)
