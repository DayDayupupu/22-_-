...
from model.CIRNet_Dense import CIRNet_Dense121
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        for i, (images, depths, gts) in enumerate(train_loader, start=1):
            ...
            # # 调整输出的大小
            # target_size = (352, 352)  # 目标大小
            # s_rgb = F.interpolate(s_rgb, size=target_size, mode='bilinear', align_corners=False)
            # s_depth = F.interpolate(s_depth, size=target_size, mode='bilinear', align_corners=False)
            # s_rgbd = F.interpolate(s_rgbd, size=target_size, mode='bilinear', align_corners=False)
...
if __name__ == '__main__':
    ...
    save_list_to_csv(loss_list, "data.csv")
    save_list_to_csv(time_list, "time.csv")
